  ...
  render(h, context) {
    let children = this.$slots.default.filter((item) => {
      return (
        ...
        item.componentOptions.tag === "c-collapse-panel"
      );
    });
    return h(
      Collapse,
      ...
      [
        ...
        children.map((child) => {
          ...
          if (child.data?.key) {
            property.key = child.data.key;
          }
          let c_children = [...child.componentOptions.children];
          if (child.data.scopedSlots?.extra) {
            c_children.push(
              h("template", { slot: "extra" }, child.data.scopedSlots.extra())
            );
          }
          if (child.data.scopedSlots?.header) {
            c_children.push(
              h("template", { slot: "header" }, child.data.scopedSlots.header())
            );

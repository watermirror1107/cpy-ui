<template>
  <div class="c_table_filter_expand" :style="{'width':width+'px','right':`-${width+10+extraRight}px`}">
    <div @click.stop="changeSelect(item)"
         :class="{'c_table_filter_expand_item_active':(isMultiple?ArrayContain(selectId,item.id):(selectId==item.id))}"
         class="c_table_filter_expand_item" v-for="(item,index) in options" :key="index">
      <span>{{ item.name }}</span>
      <c-icon v-show="isHasChild(item)" class="c_table_filter_expand_item_icon" name="icon-fanhui"/>
      <c-icon v-show="!isHasChild(item)&&(isMultiple?ArrayContain(selectId,item.id):(selectId==item.id))"
              class="c_table_filter_expand_item_icon_yes" style="font-size:10px" name="icon-xuanxiangka_gou"/>
...
export default {
  ...
  mounted() {
    this.selectId = JSON.parse(JSON.stringify(this.value));
  },
  watch: {
    value: {
      handler() {
